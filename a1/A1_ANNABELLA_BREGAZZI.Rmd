...
d <- DGEList(counts = filtered_matrix, group = types_df$subtype)
...
```{r}
par(mfrow=c(1,2))
filt_cpm <- log2(edgeR::cpm(filtered_matrix))
filt_norm <- log2(normalized_counts)

boxplot(filt_cpm, xlab="Samples", ylab="log2(CPM)",
        las = 2, cex = 0.5, cex.lab = 0.5,
        cex.axis = 0.5, main = "Original")
abline(h = median(apply(filt_cpm, 2, median)), 
       col = "red", lwd = 0.6, lty = "dashed")

boxplot(filt_norm, xlab="Samples", ylab ="log2(CPM)", 
        las = 2, cex = 0.5, cex.lab = 0.5,
        cex.axis = 0.5, main = "Normalized")
abline(h = median(apply(filt_norm, 2, median)), 
       col = "red", lwd = 0.6, lty = "dashed")

```
```{r}
density_counts <- apply(log2(edgeR::cpm(filtered_matrix)), 2, density)


xlim <- 0; ylim <- 0
for (i in 1:length(density_counts)) {
  xlim <- range(c(xlim, density_counts[[i]]$x)); 
  ylim <- range(c(ylim, density_counts[[i]]$y))
}
cols <- rainbow(length(density_counts))
ltys <- rep(1, length(density_counts))
xlim[1] = -10

plot(density_counts[[1]], xlim=xlim, ylim=ylim, type="n", 
     ylab="Smoothing density of log2(CPM)", main="Original", cex.lab = 0.8)
for (i in 1:length(density_counts)) lines(density_counts[[i]], col=cols[i])
legend("topright", colnames(filt_cpm),  
...
```{r}
norm_density_counts <- apply(log2(normalized_counts), 2, density)
xlim <- 0; ylim <- 0
for (i in 1:length(norm_density_counts)) {
  xlim <- range(c(xlim, norm_density_counts[[i]]$x)); 
  ylim <- range(c(ylim, norm_density_counts[[i]]$y))
}
xlim[1] = -10
cols <- rainbow(length(norm_density_counts))
ltys <- rep(1, length(norm_density_counts))
plot(norm_density_counts[[1]], xlim=xlim, ylim=ylim, type="n", 
     ylab="Smoothing density of log2-CPM", 
     main="Figure 2: Smoothing density of filtered and normalized CPM", cex.lab = 0.8)
for (i in 1:length(norm_density_counts)) lines(norm_density_counts[[i]], col=cols[i])
legend("topright", colnames(filt_norm),  
       col=cols, lty=ltys, cex=0.5, 
       border ="blue",  text.col = "green4", 
       merge = TRUE, bg = "gray90")
```
...
limma::plotMDS(normalized_counts, labels=NULL, pch=1,
               col = c("green", "blue", "pink", "purple")[factor(types_df$subtype)])
title("Figure 2: MDS Plot for filtered, normalized CPM by clinical subtype")
legend("bottomright", legend=levels(factor(types_df$subtype)),
       pch=c(1), col=c("green", "blue", "pink", "purple"), title="Clinical Subtype",
       bty = 'n', cex = 0.75, border="blue", bg="gray90")
...
title("Figure 2: Biological Coefficient of Variation for Clinical Subtypes")
...
```{r}
her2p <- types_df[types_df$`her2+` == TRUE,]
data_subset <- raw_counts[,her2p$sample]

data_matrix_subset <- as.matrix(data_subset)
keep <- rowSums(cpm(data_matrix_subset) > 1) > min_num
filtered_matrix_subset <- data_matrix_subset[keep,]

d_subset = DGEList(counts = data_matrix_subset,
                   group = sample_subset$subtype)
d_subset <- calcNormFactors(d_subset)
model_design <- model.matrix(~sample_subset$subtype)
d_subset <- estimateDisp(d_subset, model_design)


plotMeanVar(d_subset, show.raw.vars = TRUE,
...
write.table(normalized_counts,
            file.path(getwd(), geo_id,
                      paste(geo_id,
                            "normalized_counts.txt",
                            sep="_")),
            quote=FALSE,sep="\t",row.name=TRUE)
```
